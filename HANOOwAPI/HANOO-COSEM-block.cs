...
namespace COSEM
{
    public class HAN_OO_COSEM
    {
        ...
        public static async Task HANOOCOSEMBlock(byte[] DLMSBlock, OOUserConfigurationParameters OOuCP )
        {
            ...
            ushort dLMSfcs;
            int dlmsLength = DLMSBlock.Length;
            byte[] CRCBlock = new byte[dlmsLength - 4];
            byte[] oBISBytes = new byte[ dlmsLength - 21 ];
            ...
            // obis.showObisValues();
            await HAN_OBIS.obisCodesClass.oBISBlock( oBISBytes, OOuCP );
