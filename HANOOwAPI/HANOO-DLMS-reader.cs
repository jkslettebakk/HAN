...
namespace HANDLMS
{
    public class OO_HAN_Read_DLMS
    {
        ...
        static bool DLMS_start = false;
        static List<byte> DLMSBlock = new List<byte>();
        static COSEM.HAN_OO_COSEM Cosem = new COSEM.HAN_OO_COSEM();

        
        // public void OO_HAN_Read_DLMS_Data( OOUserConfigurationParameters OOuCP )
        public static async Task  OO_HAN_Read_DLMS_Data( OOUserConfigurationParameters OOuCP )
         {
                ...
                if ( HANBufferLength > 0 )
                {
                    ...
                    if ( (HANBufferLength >=2) && (HANData[0] == DLMSflag) && ( (HANData[1] & frameFormatTypeMask) >> 4) == iFrame  )
                    {
                        ...
                        if ( (HANData[HANBufferLength-1] == DLMSflag) )
                        {
                            ...
                            DLMS_start = false;
                            await COSEM.HAN_OO_COSEM.HANOOCOSEMBlock( DLMSBlock.ToArray(), OOuCP );
                            DLMSBlock.Clear();
                        }
                    }
                    else if ( DLMS_start )
