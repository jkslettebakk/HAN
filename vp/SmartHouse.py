...
NetAtmoPy = "/home/pi/Documents/vp/py4netatmo.py"
SmartHousePy = "/home/pi/Documents/vp/SmartHouse.py"
...
resultJsonFile = "/home/pi/Documents/vp/netAtmoResult.json"
#
...
# ready to get netatmo data
    if (writeJsonDataToDatabase):
        ...
        try:
            wetherStreamData = subprocess.Popen(['python3', NetAtmoPy]).wait()
        except wetherStreamData.exceptions as e:
            print('Error in python NetAtmo subprosess: {}. Error message = {}'.format(wetherStreamData,e))
        # Reading last netAtmo data file
        try:
            with open( resultJsonFile, "r" ) as resultJsonFileRead:
                resultJsonFileData = json.loads(resultJsonFileRead.read())
            resultJsonFileRead.close()
            inDoorTemperature = resultJsonFileData[0]["indorTemperature"]["value"]
            outDoorTemperature = resultJsonFileData[0]["outdorTemperature"]["value"]
            jsonString = jsonString + \
                ',\n\t\"{0}\": {1}'.format('indorTemperature',resultJsonFileData[0]["indorTemperature"]["value"])
            jsonString = jsonString + \
                ',\n\t\"{0}\": {1}'.format('outdorTemperature',resultJsonFileData[0]["outdorTemperature"]["value"])
        except Exception as e:
            print("Netatmo data read error from file {}.\nError = {}".format(resultJsonFile,e))

        print("Sucsessfully read netatmo indor ({}) and outdor ({}) temperature.".format(inDoorTemperature, outDoorTemperature))
    ...
    print('\nTime is: {0}'.format(datetime.datetime.now()))
